...
if ($_SERVER['REQUEST_METHOD'] === 'GET')
{
   ...
   $ip_list = $pdo->query("SELECT * FROM ip")->fetchAll(); // SELECT * FROM ip ORDER BY IPv4
   $locations = $pdo->query("SELECT * FROM locations ORDER BY name")->fetchAll();
   
   if (isset($_GET['addDevice']))
   {
      echo "\n<form action='src/dev.php?mode=add' method='post'>";
   }
   else if (isset($_GET['updateDevice']))
   {
      echo "\n<form action='src/dev.php?mode=update' method='post'>";
   }
   ...
   // Category
   echo "\n<tr><td><label>{$cfgLang['Category']}</label></td><td>";
   echo "\n<select name='category_id' id='category'>";
   echo "\n<option value=''>-- Select category --</option>";
   foreach ($categories as $cat)
   {
      $selected = $cat['id'] == $device['category_id'] ? 'selected' : '';
      echo "\n<option value='". htmlspecialchars($cat['id']) . "' $selected>". htmlspecialchars($cat['name'])."</option>";
   }
   echo "\n</select>";
   echo "</td></tr>";
   
   echo "\n<tr><td><label>{$cfgLang['Inventory']}</label></td><td><input type='text' name='inventory' value='". ($newDevice ? "'" : htmlspecialchars($device['inventory']))."'></td></tr>";
   echo "\n<tr><td><label>{$cfgLang['SN']}</label></td><td><input type='text' name='sn' value='". ($newDevice ? "'" : htmlspecialchars($device['sn']))."'></td></tr>";
   
   // IP Address 1
   echo "\n<tr><td><label>{$cfgLang['IP1']}</label></td><td>";
   echo "\n<select name='ip_id' id='ip'>";
   echo "\n<option value=''>-- Select IP --</option>";
   foreach ($ip_list as $ip)
   {
      $selected = $ip['ID'] == $device['ip_id'] ? 'selected' : '';
      echo "\n<option value='". htmlspecialchars($ip['ID']) . "' $selected>". htmlspecialchars($ip['IPv4'])."</option>";
   }
   echo "\n</select>";
   echo "\n<script>
   ...
   // IP is active
   echo "\n<tr><td><label>{$cfgLang['IPActive']}</label></td><td><input type='hidden' name='ip_isactive' value='0'>";
   if ($newDevice === true)
   {
      echo "\n<input type='checkbox' name='ip_isactive' value='1'>";
   }
   if ($newDevice === false)
   {
      echo "\n<input type='checkbox' name='ip_isactive' value='1'";
   ...
   echo "\n<tr><td><label>{$cfgLang['MAC']}</label></td><td><input type='text' name='mac' value='". ($newDevice ? "'" : htmlspecialchars($device['mac']))."'></td></tr>";
   echo "\n<tr><td><label>{$cfgLang['MAC2']}</label></td><td><input type='text' name='mac2' value='". ($newDevice ? "'" : htmlspecialchars($device['mac2']))."'></td></tr>";
   echo "\n<tr><td><label>{$cfgLang['BT']}</label></td><td><input type='text' name='bt' value='". ($newDevice ? "'" : htmlspecialchars($device['bt']))."'></td></tr>";
   echo "\n<tr><td><label>{$cfgLang['PhoneNumber']}</label></td><td><input type='text' name='phone_no' value='". ($newDevice ? "'" : htmlspecialchars($device['phone_no']))."'></td></tr>";
   ...
   foreach ($locations as $location)
   {
      ...
      echo "\n<option value='". htmlspecialchars($location['id']) . "' $selected>". htmlspecialchars($location['name'])."</option>";
   }
   echo "\n</select>";
   echo "</td></tr>";
   
   
   echo "\n<tr><td><label>{$cfgLang['Location2']}</label></td><td><input type='text' name='location2' value='". ($newDevice ? "'" : htmlspecialchars($device['location2']))."'></td></tr>";

   // Status
   echo "\n<tr><td><label>{$cfgLang['Status']}</label></td><td>";
   echo "\n<select name='status_id' id='status'>";
   echo "\n<option value=''>-- Select status --</option>";
   foreach ($statuses as $status)
   {
      $selected = $status['id'] == $device['status_id'] ? 'selected' : '';
      echo "\n<option value='". htmlspecialchars($status['id']) . "' $selected>". htmlspecialchars($status['name'])."</option>";
   }
   echo "\n</select>";
   echo "</td></tr>";
   echo "\n<tr><td><label>{$cfgLang['Purchased']}</label></td><td><input type='date' name='purchased' value='". ($newDevice ? "'" : htmlspecialchars($device['purchased']))."'></td></tr>";
   echo "\n<tr><td><label>{$cfgLang['Disposed']}</label></td><td><input type='date' name='disposed' value='". ($newDevice ? "'" : htmlspecialchars($device['disposed']))."'></td></tr>";
// echo "\n<tr><td><label>{$cfgLang['Notes']}</label></td><td><textarea name='notes' rows='4' value='". ($newDevice ? "'" : htmlspecialchars($device['notes']))."'></textarea></td></tr>";
   echo "\n<tr><td><label>{$cfgLang['Notes']}</label></td><td><textarea name='notes' rows='4'>". ($newDevice ? "" : htmlspecialchars($device['notes']))."</textarea></td></tr>";
   echo "</table>";
